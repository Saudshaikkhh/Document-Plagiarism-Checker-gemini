...
def main_plagiarism_pipeline(uploaded_file, progress_callback=None):
    ...
    try:
        ...
        try:
            ...
            sample_content = next(iter(ac_sections.values()))
            domain = detect_document_topic(sample_content)
            ...
            for i, (ac_num, content) in enumerate(ac_sections.items()):
                ...
                ai_response = gemini_plagiarism_check(ac_num, content, domain)
                parsed_result = parse_ai_response(ai_response)
                
                # Convert to format expected by UI
                plagiarism_results[ac_num] = {
                    'similarity_score': int(parsed_result['score'].replace('%', '')),
                    'confidence_level': 'High' if parsed_result['level'] == 'High' else 'Medium' if parsed_result['level'] == 'Medium' else 'Low',
                    'risk_assessment': parsed_result['level'],
                    'primary_concerns': [parsed_result['feedback'][:100]],
                    'detailed_analysis': parsed_result['feedback'],
                    'recommendations': f"Review recommended for A.C. {ac_num}",
            ...
            # Create traditional report format
            ac_results_for_pdf = {}
            for ac_num, result in plagiarism_results.items():
                ac_results_for_pdf[ac_num] = {
                    'plagiarism': 'Yes' if result['similarity_score'] >= 61 else 'No',
                    'score': f"{result['similarity_score']}%",
                    'level': result['risk_assessment'],
                    'feedback': result['detailed_analysis']
                }
            
            report_text = generate_report(ac_results_for_pdf, domain)
            
            # Generate PDF in memory
            report_buffer = BytesIO()
            save_report_to_pdf(report_text, report_buffer, domain)
